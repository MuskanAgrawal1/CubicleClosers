...
def Skills(uid):
    ...
    lblField = Label(frame2, text='Select your field of interest:', font=('arial', 17, 'bold'), bd=12, fg='maroon')
    lblField.grid(row=1, column=0, sticky=W,padx=30,pady=10)
    ...
    def my_show(uid) :
        opt=(options.get())
        print(opt)
        query = "UPDATE JSdetails SET interest=%s where jEmail=%s"
        val = (opt,uid)
    ...
    def choose(opt):
        ...
        lblRole = Label(frame3, text='Select your desired role:', font=('arial', 17, 'bold'), bd=12,fg='maroon')
        lblRole.grid(row=1, column=0, sticky=W, padx=35, pady=10)
        ...
        def my_show2(uid) :
            opt2 = (options2.get())
            query = "UPDATE JSdetails SET role=%s where jEmail=%s"
            value = (opt2,uid)
    ...
    btnNext = Button(frame4, text="Next", font=('arial', 14, 'bold'),  fg='maroon', bd=12, pady=14, padx=12,
                     width=7,
                     command=lambda: jobseeker_login.jobSeekerLog()).grid(row=0, column=2)
    root.mainloop()
