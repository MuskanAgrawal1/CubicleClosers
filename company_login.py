...
def find(uid):
    q = "select cEmail, cPassword from comdetails where cEmail ='" + str(uid) + "'"
    mycursor.execute(q)
...
def check(Email, Password):
    if(Email=="" or Password==""):
        ...
        try :
            if Password == a[0][1] :
                Company_Options.CO(Email)
            else :
                MessageBox.showerror("Error", "User: " + str(Email) + " doesn't exist.")
        except IndexError :
...
def companyLog():
        ...
        btnSignUp = Button(frame3, text="Sign Up", font=('arial', 14, 'bold'), fg='black', bd=9, pady=5,padx=12, width=7,
                         command=lambda : company_details.company()).grid(row=0, column=2)
        btnNext = Button(frame4, text="Next", font=('arial', 14, 'bold'),bg='green',fg='white', bd=12, pady=14, padx=12, width=7,
        ...
        btnExit = Button(frame4, text="Exit", font=('arial', 14, 'bold'),bg='red',fg='white', bd=12, pady=14, padx=12, width=7,
